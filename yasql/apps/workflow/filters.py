...
from workflow.models import TicketFlow
...
class TicketFlowFilter(django_filters.rest_framework.FilterSet):
    ...
    def search_action(self, queryset, name, value):
        if value == 1:  # 我创建的工单
            ...
            return queryset.filter(Q(tf_user__username=self.request.user.username) & Q(tf_user__process=False))
        elif value == 3: # 我已经处理的工单
            return queryset.filter(Q(tf_user__username=self.request.user.username) & Q(tf_user__process=True))
        else:  # 与我相关的全部工单
            return queryset.filter(Q(creator=self.request.user.username)|Q(participant=self.request.user.username)|Q(tf_user__username=self.request.user.username))
